export UPSTREAM_GERRIT_HOST_PUBLIC_KEY="ssh-rsa "
export GIT_EMAIL='openstack.ci@solidfire.com'
export GIT_NAME='SolidFire Jenkins'
export UPSTREAM_GERRIT_USER=sfci
export UPSTREAM_GERRIT_SSH_KEY_PATH=sfci_rsa.pub
export JENKINS_SSH_KEY_PATH=jenkins_key
export PUBLISH_HOST=127.0.0.1
